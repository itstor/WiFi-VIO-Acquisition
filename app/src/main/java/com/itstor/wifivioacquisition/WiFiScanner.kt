 ...
 */
class WiFiScanner(private val context: Context, private val onSuccessfulScan: (List<ScanResult>, Long) -> Unit) {
    ...
    init {
        ...
        wifiScanReceiver = object : BroadcastReceiver() {
            override fun onReceive(context: Context, intent: Intent) {
                ...
                /**
                 * WiFi scan take a while to complete.
                 * And sometimes, the scan results are not updated even though the scan is successful
                 * Because Android 9 and above only allows 4 scans per 2 minutes. @see https://developer.android.com/guide/topics/connectivity/wifi-scan
                 * This makes the sensor and VIO data not aligned with the WiFi data.
                 * To fix this, we store the timestamp of the latest successful WiFi scan
                 */
                if (success) {
                ...
                if (intent.action == WifiManager.SCAN_RESULTS_AVAILABLE_ACTION) {
                    onSuccessfulScan(getScanResults(), latestSuccessfulScanTimestamp)
                }
            }
        }
    }
